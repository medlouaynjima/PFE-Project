...
def load_memory_node(state: AgentState, config: RunnableConfig, store: BaseStore):
    """Load adherent/médecin memory from the store before starting the conversation."""
    # Create a copy of the state to prevent any unintended modifications
    new_state = state.copy()
    
    # Save the original question before passing to memory manager
    original_question = new_state.get("question", "")
    print(f"[load_memory] Preserving original question: '{original_question}'")
    
    # Load the memory
    result = load_profile_memory(new_state, config, store)
    
    # Ensure the original question is retained
    if original_question:
        result["question"] = original_question
        print(f"[load_memory] Restored original question: '{original_question}'")
    
    return result

def update_memory_node(state: AgentState, config: RunnableConfig, store: BaseStore):
    """Update adherent/médecin memory from the conversation before ending."""
    return update_profile_memory(state, config, store)
...
def process_input(state: AgentState) -> AgentState:
    ...
    # Create a fresh state, but preserve conversation history for summarization
...
def route_from_conversational(state: AgentState) -> str:
    ...
    if state.get("needs_claims", False):
        ...
        print("[ROUTER] Question requires database processing, routing to database agent")
...
def route_from_claims(state: AgentState) -> str:
    if state.get("needs_database", False):
        ...
        return "database_agent"
    elif state.get("claims", {}).get("needs_human", False):
...
def create_initial_state(question="", adherent_id="", medecin_id="", adherent_name="", medecin_name=""):
